...
This is the active frontend. The earlier `../../frontend` Vite prototype is
kept as a legacy reference and is not used by the application.

1. From the repository root, start FastAPI: `uvicorn backend.app.main:app --reload --port 8000`
2. From this folder, start the dashboard: `npm start`
3. Open `http://localhost:5173`

The dashboard calls `http://127.0.0.1:8000/api` by default and maps the real
agent feed, status, and rejected-topic endpoints into its views.

> Important: use port 5173 for the dashboard. Port 8000 is reserved for the
> FastAPI backend.
